...
def plot1d():
    ...
    trace1 = go.Scatter(x=x_data, y=np.sin(x_data))
...
def plot2d():
    ...
    trace3 = go.Histogram(x=x, name="x density", marker=dict(color="blue"), yaxis="y2")
    trace4 = go.Histogram(y=y, name="y density", marker=dict(color="blue"), xaxis="x2")
...
def plot1d_multiple(n):
    ...
    layout = go.Layout(
        height=600,
        xaxis=dict(autorange=True, title="Angle (radians)"),
        yaxis=dict(autorange=True),
    )
    fig = go.Figure(data=data, layout=layout)
    plot_div = plot(fig, output_type="div", include_plotlyjs=False)
